...
class Torch2D3D(Dataset):
    ...
    def get_h5_handle(self, key):
        if not hasattr(self, 'h5_handles'):
            self.h5_handles = {}
        if key not in self.h5_handles:
            self.h5_handles[key] = h5py.File(key, 'r', rdcc_nbytes=1024*1024, rdcc_nslots=1000, swmr=True)
        return self.h5_handles[key]
    def __getitem__(self, idx):
        while True:
            ...
            query_feats = self.load_query_features_fast(query, self.get_h5_handle(scene_info["feats_2d_path"]))
            ...
            p3d_feats = self.load_3d_features_fast(visible_p3d, self.get_h5_handle(scene_info["feats_3d_path"]))
    ...
    def load_query_features_fast(self, img_name, h5_handle):
        ds = h5_handle[img_name]
        return {
    ...
    def load_3d_features_fast(self, points3d, h5_handle):
        descriptors = []
        keypoints = []
        for id in points3d:
            id_str = str(id)
            if id_str in h5_handle:
                ds = h5_handle[id_str]
                descriptors.append(ds["descriptors"][:])
                keypoints.append(ds["keypoints"][:])
        if len(descriptors) == 0:
            return {"descriptors": np.zeros((0, 256)), "keypoints": np.zeros((0, 3))}
        
        return {
            "descriptors": np.array(descriptors).reshape(-1, 256),
...
class MegaDepth2D3D(BaseDataset):
    ...
    default_conf = {
        ...
        "split_train": "splits/train_scenes_clean.txt",
    ...
    def _build_dataset(self, split_file):
        ...
        for scene in scenes:
            ...
            #     query_names_file = query_path / "query_image_names_0_100.txt"
            # else:
