...
from ..utils.losses_sig import NLLLoss
...
def add_gaussian_noise(kpts1, std=0.002, prob=0.5):
    if random.random() > prob: # 50% chance to apply noise
        return kpts1
    noise = torch.randn_like(kpts1) * std
    noise[:, 2] *= 1.5 # add more noise to the depth dimension
    noise = torch.clamp(noise, -3*std, 3*std) # avoid extreme noise
    return kpts1 + noise    
...
def sigmoid_log_double_softmax(
    ...
    scores[:, :m, :n] = scores0 + certainties
...
class LightGlu3D(nn.Module):
    default_conf = {
        "name": "lightglu3d_no_sigma3d",  # just for interfacing
    ...
    def forward(self, data: dict) -> dict:
        ...
        if self.training and self.conf.get("3d_rotation_aug", False):
            ...
            kpts1 = add_gaussian_noise(kpts1, std=0.005, prob=0.5)
